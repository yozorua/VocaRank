...
function PayPalIcon() {
    return <svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 24 24" fill="currentColor"><path d="M19.554 7.426c.26-1.616.003-2.718-.896-3.717C17.658 2.534 15.906 2 13.683 2H7.31a.906.906 0 0 0-.897.766L4 18.27a.545.545 0 0 0 .538.63h3.93l-.27 1.717a.476.476 0 0 0 .47.553h3.302c.394 0 .729-.287.791-.676l.033-.17.628-3.982.04-.22a.799.799 0 0 1 .79-.676h.498c3.22 0 5.74-1.308 6.477-5.093.308-1.58.149-2.9-.664-3.828a3.168 3.168 0 0 0-.009-.099z" /></svg>;
}
...
export default function AboutClient({ founder: initialFounder, initialAnnouncements, initialRoadmap, initialReports, initialContributors, isAdmin, userId }: Props) {
    ...
    const [founderForm, setFounderForm] = useState({
        ...
        paypal_url: initialFounder?.paypal_url ?? '',
    });
    ...
    const filteredReports = reports
        ...
        .sort((a, b) => {
            // Open issues first, then resolved, then closed
            const statusOrder: Record<string, number> = { open: 0, resolved: 1, closed: 2 };
            const statusDiff = (statusOrder[a.status] ?? 9) - (statusOrder[b.status] ?? 9);
            if (statusDiff !== 0) return statusDiff;
            // Within same status, sort by upvote count descending
    ...
    return (
        <>
                ...
                {editingFounder ? (
                    ...
                    <div className="glass-panel hairline-border p-6 md:p-8">
                        <div className="flex flex-col gap-4">
                            ...
                            <p className="text-xs text-[var(--text-secondary)] -mb-1">{t('donation_link_hint')}</p>
                            <div className="flex items-center gap-3">
                                <div className="w-6 flex-shrink-0 text-[var(--text-secondary)] flex items-center justify-center"><PayPalIcon /></div>
                                <div className="flex-1">
                                    <label className="text-[10px] text-[var(--text-secondary)] mb-1 block">PayPal</label>
                                    <input className={inputCls} placeholder="https://paypal.me/yourhandle" value={founderForm.paypal_url} onChange={e => setFounderForm(f => ({ ...f, paypal_url: e.target.value }))} />
                                </div>
                            </div>
            ...
            {/* ══ 5. DONATION ═════════════════════════════════════════════════════ */}
            {(founder?.paypal_url || isAdmin) && (
                <section>
                    <SectionHeader>{t('donation_section')}</SectionHeader>
                    <div className="glass-panel hairline-border p-6 flex flex-col gap-4">
                        <p className="text-sm text-[var(--text-secondary)]">{t('donation_description')}</p>
                        {founder?.paypal_url && (
                            <a
                                href={founder.paypal_url}
                                target="_blank"
                                rel="noopener noreferrer"
                                className="self-start flex items-center gap-2 px-5 py-2.5 text-sm font-semibold border border-[var(--hairline)] text-white hover:border-[var(--hairline-strong)] hover:text-[var(--vermilion)] transition-colors"
                            >
                                <PayPalIcon />
                                {t('donation_button')}
                            </a>
                        )}
                        {isAdmin && !founder?.paypal_url && (
                            <p className="text-xs text-[var(--text-secondary)] italic">{t('donation_no_link_admin')}</p>
                        )}
                    </div>
                </section>
            )}
